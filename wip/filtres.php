...
//Comparer array filtres et les filtres des profils
    $profil_id = array();
    ...
    $sqlFriends = "SELECT * FROM conversation WHERE id_user1 = '$id' OR id_user2 ='$id'";
    $alreadyFriends = array();
    $firendsImport = $connexion->query($sqlFriends);
    while($row = $firendsImport->fetch_assoc()) {
        if($id == $row['id_user1']) {
            array_push($alreadyFriends, $row['id_user2']);
        } else if ($id == $row['id_user2']) {
            array_push($alreadyFriends, $row['id_user1']);
        }
    }
    
    for($i ; $i < count($alreadyFriends); $i++) {
        for($j; $j < count($profil_id); $j++) {
            if($alreadyFriends[$i] == $profil_id[$j]) {
                array_slice($profil_id, $profil_id[$j], 1);
                array_slice($profil_id, $profil_tags[$j], 1);
            }
        }
    }
